...
var SHOW_RESERVATION_INFO = 0; //0 don't show and 1 show 
...
//Function D5: Update Reservation Checkbox across all Pages 
...
var lobby_display_listen_for_selected_user = function() {
	...
	//var currently_selected_checkbox = 
	//Call Database and check if there is an active reservation 
	//console.log(" Current Reservation " + RESERVATION_NUMBER);
	...
	function(data) {
		...
		//$(".js-dispatch-active-vehicle").attr('checked', false);
		$("#js-dispatch-active-vehicle_" + selected_reservation_number).attr('checked', true);

		
		console.log(selected_reservation_number);
...
//Function D3: Call Database to Update Reservations and Listen 
...
$(document).ready(function() {            
	$(".js-dispatch-active-vehicle").change(function() {
		...
		if(this.checked) {
			...
			//console.log("checked " + current_reservation_id_full);		
		...
		//Update Reservation to Not Active (Unchecked)
			...
			//console.log("unchecked " + current_reservation_id_full);
...
var check_for_active_reservation = function() {
	...
	function(data) {
		...
		if(display_user_info.localeCompare("true")==0){ 
			...
			//PART 1: Set All Variables from reservation object 
			var reservation_number 		= reservation_details_json.reservation_no; 
			var rental_operator 		= reservation_details_json.rental_operator; 
			rental_operator 			= toTitleCase(rental_operator);
			var destination_city 		= reservation_details_json.destination_city; 
			...
			//PART 2: Set Map
			var current_city_compare = CURRENT_CITY.toUpperCase();
			var updated_city_compare = destination_city.toUpperCase();
			
			if(current_city_compare.localeCompare(updated_city_compare) == 0){
				console.log(current_city_compare + " " + updated_city_compare + " don't update ");
			} else {
				//console.log(current_city_compare + " " + updated_city_compare + " update ");	
				var loc = "https://www.google.com/maps/embed/v1/directions?key=AIzaSyAabchXVp_e81NIvOxfmWgZFrs2P7QDGNo&origin=Corvallis,Oregon&destination=" + destination_city + "," + destination_state + "&avoid=tolls|highways";
				document.getElementById('js-current-user-trip').src = loc;					
				CURRENT_CITY = destination_city;			
				js-lobby-user-trip-map
			}			
			
			//PART 3: Show Active User Area Instead of Dispatch Table
			$("#dispatch").hide();	
		...
		//NORMAL DISPLAY: Item is Not Checked (Normal Display) 
			...
			SHOW_RESERVATION_INFO = 0; 		
			//location.reload(); 
			console.log(total_time);	
			//location.reload(); 
			//PART 1: Show Dispatch Table Instead of Active User Area 			
			console.log("NOT CHECKED ");		
