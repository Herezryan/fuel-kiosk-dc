...
<body>
<!---This version 3-3-2024 implements lookups for EQNO, PIDINFO, and ACCTCODE by fuelsite and uses the autosuggest feature that was depricated due to the YUI library absence------>											
...
<cfelseif form.page eq 2>
	<cfif #fuel_site# eq "MP">
		...
		<cfelse>
			<cfoutput>
			<cfquery name="totalizer_top1" datasource="proto">
				SELECT Max(FTK_bulkfuel.id) AS MaxOfid, FTK_bulkfuel.loc_code, FTK_bulkfuel.fuel_type
				FROM FTK_bulkfuel
			...
			<cfquery name="totalizer_lookup" datasource="proto">
				SELECT FTK_bulkfuel.id, FTK_bulkfuel.loc_code, FTK_bulkfuel.fuel_type, Max(FTK_bulkfuel.totalizer_end) AS MaxOftotalizer_end
				FROM FTK_bulkfuel
				GROUP BY FTK_bulkfuel.id, FTK_bulkfuel.loc_code, FTK_bulkfuel.fuel_type
				HAVING ((FTK_bulkfuel.id)='#totalizer_top1.MaxOfID#') 
				</cfquery>				
				...
				<cfform name="form" action="#cgi.SCRIPT_NAME#?form.page=2" method="post">
					...
					<table style="width: 575px; font-size:20px;">
						...
						<tr align="left">
							...
							<td><cfinput type="submit" name="totalizer_verify" value="YES" style="height:50px; width:75px; font-size:20px;"/> <cfinput type="submit" name="totalizer_verify" value="NO" style="height:50px; width:75px; font-size:20px;"/> <a href="#cgi.SCRIPT_NAME#"> RESET</a></td>
							</tr>
						</table>
					</cfform>	
				</cfoutput>	
	...
	<cfoutput>
		<cfform name="form" action="#cgi.SCRIPT_NAME#?form.page=3" method="post">
			...
			<table style="width: 575px; font-size:20px;">
				...
				<tr align="center">	
					<td colspan="2"><h3>Fuel Site Entry Log</h3></td>
					</tr>
				<tr>	
					<td>Date/Time Insert</td>
					<td><cfinput type="hidden" name="datetime_insert" value="#dt#" style="width: 100%;">#dt#</td>
					</tr>
				<tr>
				<tr>	
					<td>Date</td>
				...
				<tr>	
					<td>Totalizer Start (xxx.x)</td>
					<cfif totalizer_verify eq "YES">
						<td><cfinput type="hidden" name="totalizer_start" value="#totalizer_start#" style="width: 100%;">#totalizer_start#</td>
					<cfelse>
						<td><cfinput type="number" step=".1" name="totalizer_start" value="#totalizer_start# "style="width: 75px;" validate="range" range=".1,9999" required="Yes" message="* You must enter starting totalizer reading." placeholder="0.0" maxlength="8"> <---- UPDATE</td>
							<cfinput type="hidden" name="totalizer_update" value="Update" />
						</cfif>
					</tr>
				<tr>
					<td>Fuel Site</td>
					<td><cfinput type="hidden" name="fuel_site" value="#fuel_site#" />#fuel_site#</td>
					</tr>
				<tr>
					<td>Fuel Type</td>
					<td><cfinput type="hidden" name="fuel_type" value="#fuel_type#" />#fuel_type#</td>
					</tr>
				<cfquery datasource="proto" name="lookup_eqno">
					SELECT FTK_bulkfuel.eq_no, Count(FTK_bulkfuel.eq_no) AS CountOfeq_no
					FROM FTK_bulkfuel
					<!---WHERE (((FTK_bulkfuel.datetime_Insert)>'1/1/2024'))--->
					GROUP BY FTK_bulkfuel.eq_no, FTK_bulkfuel.loc_code
					HAVING ((FTK_bulkfuel.loc_code)='#fuel_site#');	
					</cfquery>	
				<tr>	
					<td>EQ License or Desc.</td>
					<cfif lookup_eqno.recordCount GT 0>
					<td><cfinput type="text" name="unit_identifier" autoSuggest="#valueList(lookup_eqno.eq_no)#" typeahead="false" style="width: 100%;" required="yes" message="* Veh. ID/or Desc. Please enter an equipment unit number or description." maxlength="50"/></td>
					<cfelse>
					<cfset pid_info = " ">
					<td><cfinput type="text" name="unit_identifier" value=" " style="width: 100%;" required="yes" message="* Veh. ID/or Desc. Please enter an equipment unit number or description." maxlength="50"/></td>
					</tr>
					</cfif>
				<cfquery datasource="proto" name="lookup_pidinfo">
					SELECT FTK_bulkfuel.pid_info, Count(FTK_bulkfuel.pid_info) AS CountOfpid_info
					FROM FTK_bulkfuel
					<!---WHERE (((FTK_bulkfuel.datetime_Insert)>'1/1/2024'))--->
				...
				<tr>	
					<td>Name, initials, etc.</td>
					<cfif lookup_pidinfo.recordCount GT 0>
					<td><cfinput type="text" id="pid_info" name="pid_info" autoSuggest="#valueList(lookup_pidinfo.pid_info)#" typeahead="false" style="width: 100%;" required="yes" message="* PID Info: Please enter personally identifiable information (i.e. Name, initials, etc)." > </td>
					<cfelse>
					<cfset pid_info = " ">
					<td><cfinput type="text" id="pid_info" name="pid_info" value=" " style="width: 100%;" required="yes" message="* PID Info: Please enter personally identifiable information (i.e. Name, initials, etc)." > </td>
					</tr>	
					</cfif>
				<tr>	
					<td>Odometer (If Required)</td>
					<td><cfinput type="number" name="odometer" value="" style="width: 125px;" placeholder="No Tenths" maxlength="6"/></td>
					</tr>
				<tr>	
					<td>Gallons Pumped (xx.x)</td>
					<td><cfinput type="number" step=".1" name="qty_fuel" value="" style="width: 75px;" validate="range" range=".1,100" required="Yes" message="* Please enter gallons pumped." placeholder="0.0" min=".1" max="100" /></td>
					</tr>
				<!---tr>	
					<td>Totalizer End (xxx.x)</td>
					<td><cfinput type="number" step=".1" name="totalizer_end" value=""style="width: 75px;" validate="range" range=".1,99999" required="Yes" message="* Please enter ending totalizer reading." placeholder="0.0" min=".1" max="9999" /></td>
					</tr--->
				<tr>	
					<td>Exp. Category: (Required)</td>
					<td><cfselect id="business_purpose" name="business_purpose" editable="yes" required="yes" message="* Please enter ending totalizer reading.">
				...
				<cfquery datasource="proto" name="lookup_acctcode">
					SELECT FTK_bulkfuel.acct_code, Count(FTK_bulkfuel.acct_code) AS CountOfacct_code
					FROM FTK_bulkfuel
					<!---WHERE (((FTK_bulkfuel.datetime_Insert)>'1/1/2024'))--->
					GROUP BY FTK_bulkfuel.acct_code, FTK_bulkfuel.loc_code
					HAVING ((FTK_bulkfuel.loc_code)='#fuel_site#') and FTK_bulkfuel.acct_code is not null;	
					</cfquery>
				<tr>	
					<td>Project or Unit</td>
					<cfif lookup_acctcode.recordCount GT 0>
					<td><cfinput type="text" name="acct_code" autoSuggest="#valueList(lookup_acctcode.acct_code)#" typeahead="false" style="width: 100%px;" placeholder="Recommended" maxlength="50" /></td>
					<cfelse>
					<cfset acct_code = " ">
...
<!---Checking the error values before loading form ---->
				...
				<cfif #qty_fuel# LT .1><cfset check_count = check_count + 1></cfif>
	...
	<!----Check values-------------->			
				...
				<cfset totalizer_end = totalizer_start + qty_fuel>
				
				<cfinput type="hidden" name="totalizer_end" value="#totalizer_end#" />
